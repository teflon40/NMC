...
import { getAccessSchedule } from '../middleware/accessSchedule.middleware';
...
export const settingsController = {
    ...
    // Get access schedule (public - so frontend can show the window)
    async getAccessSchedule(_req: Request, res: Response) {
        try {
            const schedule = await getAccessSchedule();
            res.json(schedule);
        } catch (error) {
            console.error('Get access schedule error:', error);
            res.status(500).json({ error: 'Failed to get access schedule' });
        }
    },
