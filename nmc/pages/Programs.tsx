...
const Programs: React.FC = () => {
  ...
  const openAddModal = () => {
    ...
    setFormData({ name: '', shortName: '', code: '', status: 'ACTIVE', maxTasks: 1, assessmentTypeIds: assessmentTypes.map(a => a.id) });
    setIsModalOpen(true);
  };
  ...
  const toggleAssessment = (id: number) => {
    setFormData(prev => {
      ...
      if (current.includes(id)) {
        return { ...prev, assessmentTypeIds: current.filter(x => x !== id) };
      } else {
        return { ...prev, assessmentTypeIds: [...current, id] };
      }
    });
  };
  ...
  const handleSaveProgram = async () => {
    ...
    try {
      ...
      console.error(err);
      toastError('Failed to save program');
    }
  };
