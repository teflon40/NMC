...
export const createProgram = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const { name, shortName, code, status, maxTasks, assessmentTypeIds } = req.body;
        ...
        const program = await prisma.program.create({
            data: {
                ...
                shortName,
                code,
                status: (status === 'ACTIVE' || status === 'Active') ? 'ACTIVE' : 'DORMANT',
    ...
    try {
        ...
        const program = await prisma.program.update({
            ...
            data: {
                ...
                code,
                status: (status === 'ACTIVE' || status === 'Active') ? 'ACTIVE' : 'DORMANT',
                maxTasks: maxTasks ? parseInt(maxTasks) : undefined,
                ...(assessmentTypeIds && Array.isArray(assessmentTypeIds) ? {
                    assessmentTypeLinks: {
