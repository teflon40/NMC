...
export const resultsService = {
    ...
    async submitObstetrician(data: SubmitObstetricianResult): Promise<{ result: ExamResult; needsReconciliation: boolean; reconciliationId?: string; assessments?: ExamResult[] }> {
        const response = await api.post('/results/obstetrician', data);
        return response.data;
    },

    async getObstetricianReconciliation(reconciliationId: string): Promise<ExamResult[]> {
        const response = await api.get(`/results/obstetrician/reconciliation/${reconciliationId}`);
        return response.data.assessments;
    },

    async submitObstetricianFinalResult(reconciliationId: string, selectedProcedures: number[], reconciliationNotes?: string): Promise<ExamResult> {
        const response = await api.post(`/results/obstetrician/reconciliation/${reconciliationId}/finalize`, {
            selectedProcedures,
            reconciliationNotes
        });
        return response.data.result;
