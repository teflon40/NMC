...
const ObstetricianResultsTable: React.FC = () => {
    const { error: toastError, success } = useToast();
    const [selectedProgramId, setSelectedProgramId] = useState('');
    const [searched, setSearched] = useState(false);
    const [tableSearch, setTableSearch] = useState('');
    const [page, setPage] = useState(1);
    const { handleExportExcel, handleExportPDF } = useExportJob('obstetrician');
    ...
    const { data: resultsData, isLoading, refetch } = useQuery({
        queryKey: ['flat-results', 'obstetrician', searched, selectedProgramId],
        queryFn: () => resultsService.getAll({ examType: 'OBSTETRICIAN', includeAll: true, limit: 100000 }),
        enabled: searched,
    });
    ...
    const rows = useMemo(() => {
        ...
        return list.map(r => ({
            ...
            procedure: r.procedure ?? r.caseTitle ?? '—',
            score: r.score,
            scoredBy: r.examiner?.name ?? r.creator?.name ?? '—',
            reconciledBy: r.reconciledByCreator?.name ?? '—',
            isFinalSubmission: r.isFinalSubmission,
        ...
        <div className="space-y-6 animate-[fadeIn_0.3s_ease-out]">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <h2 className="text-sm font-bold text-gray-700 uppercase mb-4 flex items-center gap-2">
                    <FileText className="w-4 h-4 text-blue-600" /> Obstetrician Results
                </h2>
                            ...
                            <table className="w-full text-sm text-left min-w-[600px]">
                                <thead className="bg-gray-50 border-b border-gray-200">
                                    <tr>
                                        ...
                                        <th className="py-3 px-4 text-xs font-bold text-gray-500 uppercase">Procedure</th>
                                        ...
                                        <th className="py-3 px-4 text-xs font-bold text-gray-500 uppercase">Reconciled By</th>
                                        <th className="py-3 px-4 text-xs font-bold text-gray-500 uppercase text-center">Actions</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-100">
                                    {pageRows.length === 0 ? (
                                        <tr><td colSpan={8} className="py-10 text-center text-gray-400 italic">No data available</td></tr>
                                    ) : pageRows.map(row => (
                                        <tr key={row.id} className="hover:bg-gray-50 group">
                                            ...
                                            <td className="py-3 px-4 text-gray-600 max-w-xs truncate" title={row.procedure}>{row.procedure}</td>
                                            <td className="py-3 px-4 font-semibold text-gray-900">{row.score}%</td>
                                            <td className="py-3 px-4 text-gray-600 uppercase text-xs">{row.scoredBy}</td>
                                            <td className="py-3 px-4 text-gray-600 uppercase text-xs">{row.reconciledBy}</td>
...
export default ObstetricianResultsTable;
