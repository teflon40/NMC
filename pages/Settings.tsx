...
const Settings: React.FC = () => {
    ...
    const [activeTab, setActiveTab] = useState<'profile' | 'system' | 'access'>('profile');
    ...
    // Access Schedule State
    const [accessSchedule, setAccessSchedule] = useState({
        ACCESS_RESTRICTIONS_ENABLED: 'false',
        ACCESS_BLOCK_WEEKENDS: 'false',
        ACCESS_OPEN_TIME: '05:00',
        ACCESS_CLOSE_TIME: '14:00',
    });
    ...
    useEffect(() => {
        fetchProfile();
        fetchSystemSettings();
        if (isAdmin) fetchAccessSchedule();
    }, [isAdmin]);

    const fetchAccessSchedule = async () => {
        try {
            const response = await api.get('/settings/access-schedule');
            const d = response.data;
            setAccessSchedule({
                ACCESS_RESTRICTIONS_ENABLED: String(d.enabled),
                ACCESS_BLOCK_WEEKENDS: String(d.blockWeekends),
                ACCESS_OPEN_TIME: d.openTime || '05:00',
                ACCESS_CLOSE_TIME: d.closeTime || '14:00',
            });
        } catch (err) {
            console.error('Failed to fetch access schedule', err);
        }
    };
    ...
    const handleSaveSchedule = async () => {
        ...
        try {
            await api.put('/settings/system', accessSchedule);
            success('Access schedule saved successfully!');
        } catch (err: any) {
            toastError(err.response?.data?.error || 'Failed to save access schedule');
        } finally {
    ...
    const tabs = [
        ...
        { id: 'access', label: 'Access Schedule', adminOnly: true },
    ] as const;
                    ...
                    <div className="space-y-6">
                        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                            <div className="flex items-center justify-between mb-1">
                                <h2 className="text-lg font-semibold text-gray-800">Examiner Access Schedule</h2>
                                <span className={`px-2 py-0.5 text-xs font-bold rounded-full ${accessSchedule.ACCESS_RESTRICTIONS_ENABLED === 'true'
                                        ? 'bg-green-100 text-green-700'
                                        : 'bg-gray-100 text-gray-500'
                                    }`}>{accessSchedule.ACCESS_RESTRICTIONS_ENABLED === 'true' ? 'Active' : 'Disabled'}</span>
                            </div>
                            <p className="text-sm text-gray-500 mb-6">Control when Examiners can submit assessments. Administrators are always granted access regardless of this schedule.</p>

                            <div className="space-y-5">
                                {/* Master toggle */}
                                <div className="flex items-center justify-between p-4 bg-blue-50 border border-blue-100 rounded-lg">
                                    <div>
                                        <p className="text-sm font-semibold text-blue-900">Enable Access Restrictions</p>
                                        <p className="text-xs text-blue-600 mt-0.5">When on, only Examiners who connect within the allowed window can submit.</p>
                                    </div>
                                    <button
                                        type="button"
                                        onClick={() => setAccessSchedule(prev => ({ ...prev, ACCESS_RESTRICTIONS_ENABLED: prev.ACCESS_RESTRICTIONS_ENABLED === 'true' ? 'false' : 'true' }))}
                                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${accessSchedule.ACCESS_RESTRICTIONS_ENABLED === 'true' ? 'bg-blue-600' : 'bg-gray-300'
                                            }`}
                                    >
                                        <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${accessSchedule.ACCESS_RESTRICTIONS_ENABLED === 'true' ? 'translate-x-6' : 'translate-x-1'
                                            }`} />
                                    </button>
                                </div>
                                ...
                                <div className="flex items-start gap-3 p-4 border border-gray-200 rounded-lg">
                                    <input
                                        id="block-weekends"
                                        type="checkbox"
                                        checked={accessSchedule.ACCESS_BLOCK_WEEKENDS === 'true'}
                                        onChange={e => setAccessSchedule(prev => ({ ...prev, ACCESS_BLOCK_WEEKENDS: String(e.target.checked) }))}
                                        className="mt-0.5 h-4 w-4 accent-blue-600 cursor-pointer"
                                    />
                                    <label htmlFor="block-weekends" className="cursor-pointer">
                                        <p className="text-sm font-semibold text-gray-800">Block Access on Weekends</p>
                                        <p className="text-xs text-gray-500 mt-0.5">Saturday and Sunday will be fully locked out regardless of the daily time window.</p>
                                    </label>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-1">Opens at (daily)</label>
                                        <input
                                            id="access-open-time"
                                            type="time"
                                            value={accessSchedule.ACCESS_OPEN_TIME}
                                            onChange={e => setAccessSchedule(prev => ({ ...prev, ACCESS_OPEN_TIME: e.target.value }))}
                                        ...
                                        />
                                        <p className="text-xs text-gray-400 mt-1">e.g. 05:00 for 5 AM</p>
                                    </div>
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-1">Closes at (daily)</label>
                                        <input
                                            id="access-close-time"
                                            type="time"
                                            value={accessSchedule.ACCESS_CLOSE_TIME}
                                            onChange={e => setAccessSchedule(prev => ({ ...prev, ACCESS_CLOSE_TIME: e.target.value }))}
                                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all text-sm"
                                        />
                                        <p className="text-xs text-gray-400 mt-1">e.g. 14:00 for 2 PM</p>
                                    </div>
                                </div>

                                {/* Preview */}
                                {accessSchedule.ACCESS_RESTRICTIONS_ENABLED === 'true' && (
                                    <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-700">
                                        <strong>Preview:</strong>{' '}
                                        Examiners can access the system{' '}
                                        {accessSchedule.ACCESS_BLOCK_WEEKENDS === 'true' ? 'Monday–Friday' : 'every day'},{' '}
                                        between <strong>{accessSchedule.ACCESS_OPEN_TIME}</strong> and <strong>{accessSchedule.ACCESS_CLOSE_TIME}</strong>.
                                    </div>
                                )}
                            </div>
